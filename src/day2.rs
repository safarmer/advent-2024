...
impl Step {
  fn new(i1: i32, i2: i32) -> Step {
    let size = i2 - i1;
    let direction = if size.is_positive() {
      Direction::Up
    } else {
      Direction::Down
    };
  ...
  fn is_safe_size(&self) -> bool {
    (MIN_STEP_SIZE..=MAX_STEP_SIZE).contains(&self.size.abs())
  }
}
...
pub fn day2a(lines: &Vec<String>) -> i32 {
  fn is_safe(report: &[i32]) -> bool {
    let step = Step::new(report[0], report[1]);
    let fact = if report[0] < report[1] { 1 } else { -1 };
    for i in 1..report.len() {
      let (step, _) = size_and_dir(report[i - 1], report[i]);
      if !is_step_size_safe(step) {
        return false;
      }
    }
    true
